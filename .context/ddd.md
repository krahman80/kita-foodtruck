...
This document defines the domain model behind the application — the core concepts, their rules, and how they relate — independent of the Laravel/Inertia/Vue implementation. It complements the PRD (what to build) and the database structure doc (how data is stored) by capturing *why the rules are what they are* and giving the team a shared vocabulary (ubiquitous language) to talk about the system without ambiguity.
...
| **Rest Day / Holiday** | Any calendar date with **no** Location Entry. Not a distinct stored record — it's the *absence* of an entry. |
...
1. Exactly **one** `LocationEntry` may exist per `schedule_date` (enforced at the aggregate/persistence boundary — this *is* the calendar).
...
5. The public site never looks beyond **today's date** — past and future entries exist for the Owner's planning/history, but the domain's public-facing query is always scoped to "does today have a scheduled entry?" (confirmed product decision — no forward-looking "next stop" logic).
...
**Repository responsibility:** Given "today," return the single matching `LocationEntry` or nothing (Rest Day). Given an event lookup, return the nearest upcoming entry where `is_event = true` (used only by the Hero's event pill, not the Location section itself).
...
4. An `is_active = false` item is entirely excluded from all public queries (Menu grid *and* Hero's popular-item lookup), even if `is_popular = true` — active status always takes precedence.
...
Since "exactly one popular item" is a constraint *across* aggregates, not within one, it's modeled as a small domain service rather than aggregate logic:
