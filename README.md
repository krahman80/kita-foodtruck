...
- `project/` — the **Laravel + Vue/Inertia application** (all app source, config, and docs below refer to this).
- `docker-compose.yml` + `docker/` — local development environment (Docker; not used in production/shared hosting).
- `.context/` — planning/design docs (PRD, DDD, ERD, progress report).

## Deployment

- **[`project/DEPLOYMENT.md`](./project/DEPLOYMENT.md)** — general shared-hosting (cPanel-style) guide.
- **[`project/HOSTINGER-DEPLOYMENT.md`](./project/HOSTINGER-DEPLOYMENT.md)** — Hostinger-specific (hPanel) guide.
...
The app runs in Docker (`docker-compose.yml` at the repo root; container named `app`, exposed on port `8020`). Run these commands from inside `project/`:

```bash
cd project
docker exec app php artisan migrate --seed
npm install
docker exec app php artisan test      # tests
npm run dev                          # Vite dev (or npm run build)
```
