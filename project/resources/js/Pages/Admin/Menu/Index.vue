                ...
                <div class="mt-8 overflow-hidden rounded-lg bg-white shadow">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="item in menuItems" :key="item.id" :class="{ 'bg-gray-50': !item.is_active }">
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600">{{ label(item.category) }}</td>
                                <td class="px-4 py-3 text-right text-sm text-gray-900">¥{{
                                    item.price_yen.toLocaleString() }}
                                </td>
                                <td class="px-4 py-3 text-center text-xs">
                                    <span class="inline-flex flex-wrap justify-center gap-1">
                                        <span class="rounded bg-orange-100 px-1.5 py-0.5 text-orange-700"
                                            title="Sold out" v-if="item.is_sold_out">Sold out</span>
                                        <span class="rounded bg-green-100 px-1.5 py-0.5 text-green-700"
                                            v-if="item.is_active">Active</span>
                                        <span class="rounded bg-gray-200 px-1.5 py-0.5 text-gray-600"
                            ...
                            <tr v-if="menuItems.length === 0">
                                <td colspan="5" class="px-4 py-8 text-center text-sm text-gray-500">No menu items yet.
                                    Add your
                                    first item above.</td>
                            </tr>
                        </tbody>
